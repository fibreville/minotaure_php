...
$nbhma=$db->query("SELECT COUNT(*) FROM hrpg WHERE mind>=str")->fetchColumn();
$nbhstr=$db->query("SELECT COUNT(*) FROM hrpg WHERE mind<str")->fetchColumn();
...
<body bgcolor="white">
<font style="font-family: 'Libre Baskerville', Black;font-size: 30px;">Notre Aventure<br><font style="font-family: 'Libre Baskerville', Black;font-size: 20px;">Rejoindre l'Aventure : maspero.blue/rpg/ ou taper !aventure</font></font>
<br><br>
...
Leader du groupe : <b><? print "$leader"; ?></b><br>
Traître du groupe : <b><? print "$traitre"; ?></b><br><br>
Joueurs encore en vie : <b><? print "$nbhv"; ?></b><br>
...
$color="000000";$femme="Homme";
if ($hf==1) {$color="990000";$femme="Femme";}
if ($hf==3) {$color="990000";$femme="Non Binaire";}

if ($tag1!="" && $tag1!=" ") {$femme.="<br>".$tag1;}
if ($tag2!="" && $tag2!=" ") {$femme.="<br>".$tag2;}
if ($tag3!="" && $tag3!=" ") {$femme.="<br>".$tag3;}

if ($str<$mind) {$aptitude="malin ($mind/6)";} 
if ($str>$mind) {$aptitude="fort ($str/6)";}
if ($str==$mind) {$aptitude="malin et fort ($str/6)";}
...
<br>Joueurs morts : <? print "$nbhm"; ?><br><br>
...
print "<td width=80><table border=0 cellspacing=1 bgcolor=black width=100% height=100%><tr><td bgcolor=white>
<font style=\"font-family: 'Libre Baskerville', Black;font-size: 12px;\"><b><font color=$color><font style=\"font-family: 'Libre Baskerville', Black;font-size: 10px;\">$nom<br></b>$femme</font></font><br>Esprit : $mind<br>Corps : $str<br>Or : $or<br>Vie : $hp<br>Joueur #$id_joueur</font></td></tr></table>
</td>";
...
<br><br><font style="font-family: 'Libre Baskerville', Black;font-size: 12px;"><? print $sanction; ?></font>
...
Designer un tag au hasard : <input type="text" name="choixrandom" size="15"><input type="submit" value="OK">
</form>
<br>
<b>Le joueur désigné est :</b> <? print "$designe"; ?><br><br><br>
<b>Actions :</b>
<br>
<br>
<? 
...
if ($c1!="") {$nbc1=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=1")->fetchColumn();
$pc1=round(($nbc1*100/$nbhv),2);
print "<tr><td>$c1 : </td><td>$nbc1 / $nbhv soit $pc1 %</td></tr>";
}

if ($c2!="") {$nbc2=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=2")->fetchColumn();
$pc2=round(($nbc2*100/$nbhv),2);
print "<tr><td>$c2 : </td><td>$nbc2 / $nbhv soit $pc2 %</td></tr>";
}
if ($c3!="") {$nbc3=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=3")->fetchColumn();
$pc3=round(($nbc3*100/$nbhv),2);
print "<tr><td>$c3 : </td><td>$nbc3 / $nbhv soit $pc3 %</td></tr>";
}
if ($c4!="") {$nbc4=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=4")->fetchColumn();
$pc4=round(($nbc4*100/$nbhv),2);
print "<tr><td>$c4 : </td><td>$nbc4 / $nbhv soit $pc4 %</td></tr>";
}
if ($c5!="") {$nbc5=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=5")->fetchColumn();
$pc5=round(($nbc5*100/$nbhv),2);
print "<tr><td>$c5 : </td><td>$nbc5 / $nbhv soit $pc5 %</td></tr>";
}
if ($c6!="") {$nbc6=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=6")->fetchColumn();
$pc6=round(($nbc6*100/$nbhv),2);
print "<tr><td>$c6 : </td><td>$nbc6 / $nbhv soit $pc6 %</td></tr>";
}
if ($c7!="") {$nbc7=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=7")->fetchColumn();
$pc7=round(($nbc7*100/$nbhv),2);
print "<tr><td>$c7 : </td><td>$nbc7 / $nbhv soit $pc7 %</td></tr>";
}
if ($c8!="") {$nbc8=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=8")->fetchColumn();
$pc8=round(($nbc8*100/$nbhv),2);
print "<tr><td>$c8 : </td><td>$nbc8 / $nbhv soit $pc8 %</td></tr>";
}
if ($c9!="") {$nbc9=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=9")->fetchColumn();
$pc9=round(($nbc9*100/$nbhv),2);
print "<tr><td>$c9 : </td><td>$nbc9 / $nbhv soit $pc9 %</td></tr>";
}
if ($c10!="") {$nbc10=$db->query("SELECT COUNT(id) FROM hrpg WHERE vote=10")->fetchColumn();
...
if ($choixtag!="") {print "(vote limité à $choixtag)<br>";}

if ($leadvalue==2) {print "<br><b>Le leader $leader a utilisé son pouvoir et choisi le choix $leadvote !</b><br>";}

if ($traitrevalue==2) {print "<br><b>Le traitre $traitre a utilisé son pouvoir et choisi d'annuler le choix $traitrevote !</b><br>";}

print "<br><a href=ecran.php>Rafraichir</a><br>
<a href=ecran.php?action=clean>Clean</a>";
...
<form method=post action=ecran.php>
<input type="text" name="choix" size="30" value="Intitulé des choix"><br>
